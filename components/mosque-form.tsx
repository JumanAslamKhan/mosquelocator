...
export default function MosqueForm() {
  ...
  const [form, setForm] = useState({
    ...
    phone: "",
    facilities: "Parking, Wudu, Women prayer area",
    description: "",
    fajr: "05:45 AM",
    dhuhr: "12:35 PM",
    asr: "04:10 PM",
  ...
  // Check auth session on mount and require login for adding mosques
  useEffect(() => {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const response = await fetch("/api/mosques", {
      ...
      body: JSON.stringify({
        ...
        owner_id: data.session?.user.id ?? null,
    ...
    setMessage("Mosque saved successfully.");
    setLoading(false);
  }
      ...
      <div className="mx-auto w-full max-w-4xl">
        <div className="rounded-[2rem] border border-white/10 bg-[#0d1f18]/95 p-6 shadow-2xl shadow-black/40 backdrop-blur-xl sm:p-8">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <p className="text-xs uppercase tracking-[0.35em] text-emerald-200/70">Add mosque</p>
              <h1 className="mt-2 text-3xl font-semibold tracking-tight">Create a new mosque listing</h1>
            </div>
          ...
          <form className="mt-8 grid gap-5 sm:grid-cols-2" onSubmit={handleSubmit}>
            ...
            <div className="sm:col-span-2 rounded-3xl border border-white/10 bg-white/5 p-4 sm:p-5">
              <div className="mb-4">
                ...
                <p className="mt-1 text-xs text-slate-400">Enter the standard daily times and optional extra Jumuah jamat slots.</p>
              </div>
            ...
            <div className="flex flex-col gap-3 sm:col-span-2 sm:flex-row">
              ...
              >
                {loading ? "Saving..." : "Save mosque"}
              </button>
              {isAuthenticated === false ? (
                <div className="mt-2 text-sm text-amber-200">You must <a href="/login" className="underline">log in</a> to add a mosque.</div>
              ) : null}
